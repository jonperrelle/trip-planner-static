...
db.sync({force: true})  // syncs every table that has been registered to the db (restaurant, etc.)
.then(function () {     // says what tables should look like - have to sync
...
.then(function () {
  ...
  return Promise.map(Object.keys(data), function (name) {
    return Promise.map(data[name], function (item) {
      return db.model(name)  // can fetch model with db.model and invoke it with string that matches models name
      .create(item, {   // this creates the tables with the items, including the place model information
        include: [Place]
      });
    });
  });
